...
- **Task size** — mark how big a task is with an open, configurable tag (default `#s` / `#m` / `#l` for small / medium / large) — set or clear it from the task's popover, no minute estimates. The size shows in the popover and is a plain tag, so Dataview or the Tasks plugin can read it too. It's the groundwork for size-aware planning (quick wins, 1-3-5, a value/effort view).
