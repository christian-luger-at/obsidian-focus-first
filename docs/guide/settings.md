...
Focus First's settings live under **Settings → Community plugins → Focus First**. Sections are grouped so you can find things quickly.
...
| **Value / Effort** | For the Value/Effort preset: the value source (priority or manual tag), the `#highvalue` / `#lowvalue` override tags, and which sizes count as low effort. |
...
Group tasks within a quadrant by priority, due date, or alphabetically, with independently configurable primary/secondary sort order per quadrant.
