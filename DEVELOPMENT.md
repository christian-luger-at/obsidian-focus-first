...
The release is published directly (not as a draft) with the locally built assets. Publishing fires the "release" event the `Attest release build` workflow needs; it then rebuilds `main.js` in CI, generates a signed build-provenance attestation for all three assets, and re-uploads them - so within about 30 seconds the public release carries CI-built, attested assets rather than the locally-built ones. Verify any asset with `gh attestation verify main.js --repo christian-luger-at/obsidian-focus-first`.

> [!note]
> Attestation was removed for a few days (2026-07-17 to 2026-07-20): Obsidian's automated review started rejecting it as cryptographically invalid, even though it verified cleanly under every `gh attestation verify` policy and was identical in form to 1.4.2, which the same review had passed. The failed check got the plugin delisted from the community store, alongside 9 other plugins removed in the same bot commit. The actual cause was on Obsidian's side: their review scanner hadn't been updated for a GitHub attestation API format change (the `bundle` field moved to `bundle_url`), confirmed and fixed by an Obsidian moderator on the forum. The version stamping in `esbuild.config.mjs` and `version-bump.mjs` stays regardless, so a release can never inherit an older release's attestation via a matching digest.
